<template>
  <span
    v-if="closeText"
    class="w-switch-closeText"
    :style="[`color:${switchValue ? '#333' : '#1890ff'}`]"
  >
    {{ closeText }}
  </span>
  ...
  <span
    ...
    @click="clickSwitch"
  >
    <span class="w-switch-roll" :style="switchRollStyle" />
  </span>

  <span
    v-if="openText"
    class="w-switch-openText"
    :style="[`color:${modelValue ? '#1890ff' : '#333'}`]"
  >
    {{ openText }}
  </span>
</template>
...
function _WSwitch () {
  ...
  const switchRollStyle = computed(() => {
    ...
    `border:1.3px solid  ${switchColor.value}`
    ]
  })
